...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score > 0:
        point_scored = digit_fn(opponent_score % 10)
        player_score = point_scored(player_score)
        opponent_score //= 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    sum_points = hefty_hogs(score, opponent_score)
    temp_score = (score + hefty_hogs(score, opponent_score))
    sum_points += hog_pile(temp_score, opponent_score)

    if sum_points >= threshold:
        return 0
    else:
        return num_rolls
    """
