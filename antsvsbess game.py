...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
            # BEGIN Problem 8
        elif place.ant.is_container and not place.ant.ant_contained and not self.is_container:
            container = place.ant
            container.ant_contained = self
        elif self.is_container and self.ant_contained == None and not place.ant.is_container:
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if self.min_range <= distance and distance <= self.max_range:
                current_bee = random_bee(place.bees)
                if not current_bee == None:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            bee_health = amount + self.damage
        else: 
            bee_health = amount
        for i in list(self.place.bees):
            Ant.reduce_health(i, bee_health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        bees_in_place = []
        for i in gamestate.bees:
            if i.place == self.place:
                bees_in_place.append(i)
        for j in bees_in_place:
            j.reduce_health(self.damage)
# END Problem 9
