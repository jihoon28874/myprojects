    ...
    public boolean tilt(Side side) {
        ...
        for (int col = 0; col < size; col++) {
            ...
            for (int row = size - 1; row >= 0; row--) {
                ...
                k = row + 1;
                if (t != null && k < size) {
                    while (_board.tile(col, k) == null && k + 1 < size) {
                        k = k + 1;
                    }
                    new_position = _board.tile(col, k);
                    if (_board.tile(col, k) == null) {
                        _board.move(col, k, t);
                        changed = true;
                    } else if (new_position.value() == t.value() && grid[k] == 0) {
                        grid[k] = 1;
                        _board.move(col, k , t);
                        changed = true;
                        _score = _score + (t.value() * 2);
                    } else {
                        _board.move(col, k - 1, t);
                        changed = true;
                    }
                }
            }
