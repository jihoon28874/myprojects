...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        proc = do_lambda_form(Pair(signature.rest,expressions.rest),env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    l = LambdaProcedure(formals, expressions.rest, env)
    return l
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    expr = expressions
    while expr != nil:
        k = scheme_eval(expr.first, env)
        if is_scheme_false(k) or expr.rest == nil:
            return k
        expr = expr.rest
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expr != nil:
        k = scheme_eval(expr.first, env)
        if is_scheme_true(k) or expr.rest == nil:
            return k
        expr = expr.rest
    # END PROBLEM 12
